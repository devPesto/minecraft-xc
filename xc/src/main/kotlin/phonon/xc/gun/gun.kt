 ...
 */
const val GUN_MAX_AMMO_ALLOWED: Int = 1024

/**
 * Gun single shot firing mode types.
 */
enum class GunSingleFireMode {
    NONE,
    SINGLE,
    BURST,
    ;
    ...
    companion object {
         ...
         */
        fun match(name: String): GunSingleFireMode? {
...
data class Gun(
    // gun type
    val type: GunType = GunType.RIFLE,

    // gun item/visual properties
    val itemName: String = "gun",
    val itemLore: List<String> = listOf(),
    val itemModelDefault: Int = 0,     // normal model (custom model data id)
    val itemModelEmpty: Int = -1,      // when gun out of ammo
    val itemModelReload: Int = -1,     // when gun is reloading
    val itemModelAimDownSights: Int = -1, // when using iron sights

    // alternative material for aim down sights model
    val materialAimDownSights: Material = Material.CARROT_ON_A_STICK,

    // death message
    // {0} = playerName,
    // {1} = killerName,
    // {2} = gun.itemName,
    val deathMessage: String = "{0} was shot by {1} using {2}",

    // equiped properties
    // slowness while equiped (if > 0)
    val equipDelayMillis: Long = 500, // delay before player can fire after selecting weapon
    val equipSlowness: Int = 0,
    val equipNoSprint: Boolean = false,

    // require crawl (prone position) to shoot
    val crawlRequired: Boolean = false,
    val crawlTimeMillis: Long = 1000, // time to crawl to shoot in milliseconds

    // reload [ms]
    val reloadTimeMillis: Long = 1500,

    // semiauto/regular shoot properties [ms]
    val singleFireMode: GunSingleFireMode = GunSingleFireMode.SINGLE,
    val burstFireCount: Int = 3,
    val burstFireDelayTicks: Int = 2,
    val burstFireDelayTickPattern: IntArray = intArrayOf(), // pattern of delay ticks
    val shootDelayMillis: Long = 500,
    // remove this item after doing single-shot firing.
    // use to make single-use guns like a panzerfaust
    val shootConsumeOnUse: Boolean = false,

    // automatic fire rate properties
    val autoFire: Boolean = false,     // automatic weapon
    val autoFireDelayTicks: Int = 2,   // auto fire rate in ticks
    val autoFireDelayTickPattern: IntArray = intArrayOf(), // pattern of delay ticks
    val autoFireSlowness: Int = 2,     // auto fire slowness level

    // ammo
    val ammoId: Int = -1,
    val ammoDesc: String? = null,
    val ammoMax: Int = 10,
    val ammoPerReload: Int = -1,       // if -1, reload to max. otherwise: ammo + ammoPerReload
    val ammoIgnore: Boolean = false,   // if true, ignores out of ammo
    
    // sway
    val swayBase: Double = 0.05,           // base sway (how random projectiles are)
    val swaySpeedMultiplier: Double = 2.0, // sway multiplier while moving, depends on player velocity
    val swayAimDownSights: Double = 0.25,  // sway while aiming down sights
    val swayRideHorse: Double = 1.5,       // sway while riding horse
    val swayRideBoat: Double = 1.5,        // sway while riding boat
    val swayRideArmorStand: Double = 1.0,  // sway while riding armor stand

    // recoil
    val recoilSingleHorizontal: Double = 2.0,
    val recoilSingleVertical: Double = 5.0,
    val recoilAutoHorizontal: Double = 2.0,
    val recoilAutoVertical: Double = 5.0,
    val recoilSingleFireRamp: Double = 0.1,
    val recoilAutoFireRamp: Double = 0.1,
    ...
    val projectileVelocity: Float = 16.0f,
    ...
    val projectileGravity: Float = 0.025f,

    // max lifetime in ticks before despawning
    val projectileLifetime: Int = 400, // ~20 seconds

    // max projectile distance in blocks before despawning
    val projectileMaxDistance: Float = 128.0f, // = view distance of 8 chunks
    
    // proximity projectiles, like flak gun rounds, explode when < distance from target
    val projectileProximity: Float = 0.0f, // distance in blocks
    ...
    val projectilePassthroughDoors: Boolean = false,

    // main projectile damage
    val projectileDamage: Double = 4.0,
    val projectileArmorReduction: Double = 0.5,
    val projectileResistanceReduction: Double = 0.5,
    val projectileDamageType: DamageType = DamageType.BULLET,
    val projectileDamageMin: Double = 4.0,
    val projectileDamageDropDistance: Double = 0.0,

    // projectile particle config
    val projectileParticleType: Particle = Particle.DUST,
    val projectileParticleSize: Float = 0.35f,
    val projectileParticleColor: Color = Color.WHITE,
    val projectileParticleSpacing: Double = 1.5,
    val projectileParticleForceRender: Boolean = true,

    // flag for projectile block hit particles and crack animation
    val projectileBlockHitParticles: Boolean = true,

    // projectile hitbox impact particles
    val projectileImpactParticles: ParticlePacket? = null,

    // explosion damage and radius and falloff (unused if no explosion)
    val explosionDamage: Double = 8.0,
    val explosionMaxDistance: Double = 8.0,        // max distance for checking entities
    val explosionRadius: Double = 1.0,
    val explosionFalloff: Double = 2.0,            // damage/block falloff
    val explosionArmorReduction: Double = 0.5,     // damage/armor point
    val explosionBlastProtReduction: Double = 1.0, // damage/blast protection
    val explosionDamageType: DamageType = DamageType.EXPLOSIVE_SHELL,
    val explosionBlockDamagePower: Float = 0f,     // explosion block damage power level
    val explosionFireTicks: Int = 0,               // if explosion should set targets on fire
    val explosionParticles: ParticlePacket = ParticlePacket.placeholderExplosion(),

    // handler on block hit
    val hitBlockHandler: GunHitBlockHandler = noBlockHitHandler,

    // handler on entity hit
    val hitEntityHandler: GunHitEntityHandler = entityDamageHitHandler,
    
    // hit fire ticks (when hit handler is fire)
    val hitFireTicks: Int = 0,
    // probability setting a hit block on fire
    val hitBlockFireProbability: Double = 0.04,
    ...
    val soundShoot: String = "gun_shoot",
    val soundShootVolume: Float = 6f,
    val soundShootPitch: Float = 1f,
    val soundEmpty: String = "gun_empty",
    val soundEmptyVolume: Float = 1f,
    val soundEmptyPitch: Float = 1f,
    val soundReloadStart: String = "gun_reload_start",
    val soundReloadStartVolume: Float = 1f,
    val soundReloadStartPitch: Float = 1f,
    val soundReloadFinish: String = "gun_reload_finish",
    val soundReloadFinishVolume: Float = 1f,
    val soundReloadFinishPitch: Float = 1f,
    val soundImpact: String? = null, // for hit entity or block handler
    val soundImpactVolume: Float = 6f,
    val soundImpactPitch: Float = 1f,
    val soundExplosion: String = "minecraft:entity.generic.explode", // for explosion handler
    val soundExplosionVolume: Float = 6f,
    val soundExplosionPitch: Float = 1f,
): IntoItemStack {
    ...
    val itemDescriptionForAmmo: List<List<String>>

    // use firing delay patterns (different delays between shots)
    val useBurstFireDelayTickPattern: Boolean = burstFireDelayTickPattern.isNotEmpty()
    val useAutoFireDelayTickPattern: Boolean = autoFireDelayTickPattern.isNotEmpty()
     ...
     */
    val attackSpeedAttributeModifierValue: Double = (1000.0 / this.shootDelayMillis.toDouble()) - 4.0
    val attackSpeedAttributeModifier: AttributeModifier = AttributeModifier(
        UUID.randomUUID(),
        "gun.attackSpeed",
    ...
    val aimDownSightsModel: AimDownSightsModel = AimDownSightsModelPacketManager(this, materialAimDownSights)
     ...
     */
    fun getItemDescriptionForAmmo(ammo: Int): List<String> {
     ...
     * damage drop.
     */
    fun projectileDamageAtDistance(distance: Double): Double {
    ...
    companion object {
         ...
         */
        fun fromToml(source: Path, logger: Logger? = null): Gun? {
            try {
         ...
         */
        fun loadFilename(filename: String, logger: Logger? = null): Gun? {
         ...
         */
        fun loadFilenameOrDefault(filename: String, default: Gun, logger: Logger? = null): Gun {
            val p = Paths.get(filename)
            if ( Files.isRegularFile(p) ) {
                try {
